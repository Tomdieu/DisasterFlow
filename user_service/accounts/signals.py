...
def publish_message(event_type, data, exchange_name="accounts"):
    try:
        ...
        topic_publish(event_type, data, exchange_name=exchange_name)
...
def create_token_and_notify_other_services(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        topic_publish(events.USER_CREATED, data, exchange_name="accounts")
        ...
        topic_publish(events.USER_UPDATED, data, exchange_name="accounts")
...
def notify_other_services(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        topic_publish(events.CITIZEN_CREATED, data, exchange_name="accounts")
        # publish_message.delay(events.CITIZEN_CREATED, data)
    else:
        # publish(events.CITIZEN_UPDATED, data)
        topic_publish(events.CITIZEN_UPDATED, data, exchange_name="accounts")
...
def notify_other_services(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        topic_publish(events.EMERGENCY_RESPONDER_CREATED, data, exchange_name="accounts")
        # publish_message.delay(events.EMERGENCY_RESPONDER_CREATED, data)
    else:
        # publish(events.EMERGENCY_RESPONDER_UPDATED, data)
        topic_publish(events.EMERGENCY_RESPONDER_UPDATED, data, exchange_name="accounts")
...
def create_location_and_notify_other_services(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        topic_publish(events.USER_LOCATION_CREATED, data, exchange_name="accounts")
        # publish_message.delay(events.USER_LOCATION_CREATED, data)
    else:
        # publish(events.USER_LOCATION_UPDATED, data)
        topic_publish(events.USER_LOCATION_UPDATED, data, exchange_name="accounts")
...
def create_profile_and_notify_other_services(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        topic_publish(events.PROFILE_CREATED, data, exchange_name="accounts")
        # publish_message.delay(events.PROFILE_CREATED, data)
    else:
        # publish(events.PROFILE_UPDATED, data)
        topic_publish(events.PROFILE_UPDATED, data, exchange_name="accounts")
...
def delete_token_and_notify_other_services(sender, instance, **kwargs):
    data = UserSerializer(instance).data
    topic_publish(events.USER_DELETED, data, exchange_name="accounts")
